...
type SubmitStatus =
  | { type: "idle" }
  | { type: "success"; appointment: AppointmentDetails }
...
function googleCalendarUrl(appointment: AppointmentDetails, title: string): string {
  const start = appointment.startIso.replace(/[-:]/g, "").replace(/\.\d{3}Z$/, "Z");
  const end = appointment.endIso.replace(/[-:]/g, "").replace(/\.\d{3}Z$/, "Z");
  const params = new URLSearchParams({
    action: "TEMPLATE",
    text: title,
    dates: `${start}/${end}`,
    details:
      appointment.consultationType === "video" && appointment.meetLink
        ? `Join Google Meet: ${appointment.meetLink}`
        : "Audio call with ROLAN AUTOMATION",
...
function downloadIcs(appointment: AppointmentDetails) {
  const stamp = appointment.startIso.replace(/[-:]/g, "").replace(/\.\d{3}Z$/, "Z");
  ...
  const ics = [
    ...
    appointment.meetLink ? `LOCATION:${appointment.meetLink}` : null,
    "END:VEVENT",
    "END:VCALENDAR",
  ]
    .filter(Boolean)
  ...
  a.href = url;
  a.download = "rolan-appointment.ics";
  a.click();
    ...
    >
      ...
      <div className="mt-6 flex w-full max-w-sm flex-col gap-2.5 sm:flex-row">
        <Button
          ...
          onClick={() => {
            if (appointment.htmlLink) {
              window.open(appointment.htmlLink, "_blank", "noopener,noreferrer");
              return;
            }
            window.open(
              googleCalendarUrl(appointment, calendarTitle),
              "_blank",
              "noopener,noreferrer"
            );
          }}
...
export function AppointmentForm() {
  ...
  const loadDates = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/appointment/availability?${params}`);
      ...
      if (!response.ok || !result?.ok) {
        setAvailableDates([]);
        setAvailabilityError(result?.error || "Unable to load available dates.");
        return;
      }

      const nextDates = result.dates || [];
      setAvailableDates(nextDates);
      const currentDate = getValues("date");
      if (currentDate && !nextDates.includes(String(currentDate))) {
        setValue("date", "");
        setValue("time", "");
        setSlots([]);
      }
      ...
      setAvailableDates([]);
      setAvailabilityError("Network error while loading dates.");
    } finally {
      setDatesLoading(false);
    }
  }, [timeZone, duration, getValues, setValue]);
  ...
  const loadSlots = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok || !result?.ok) {
        setSlots([]);
        setAvailabilityError(result?.error || "Unable to load available times.");
        return;
      }
      ...
      setSlots([]);
      setAvailabilityError("Network error while loading times.");
    } finally {
      setSlotsLoading(false);
    }
  }, [date, timeZone, duration, getValues, setValue]);
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (response.ok && result?.ok && result.appointment) {
        reset(defaultValues);
        setStatus({ type: "success", appointment: result.appointment });
        return;
      }
      ...
      <fieldset disabled={busy} className="m-0 min-w-0 space-y-5 border-0 p-0">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            ...
            <select id="appt-duration" className={fieldClass} {...register("duration")}>
          ...
          <div>
            <Label htmlFor="appt-timezone">Your time zone</Label>
            <select id="appt-timezone" className={fieldClass} {...register("timeZone")}>
        ...
        <div>
          <Label htmlFor="appt-date">Available dates</Label>
          {datesLoading ? (
            ...
            <p className="mt-2 text-sm text-muted">
              {availabilityError || "No open dates right now. Please try again later."}
            </p>
          ) : (
            <select id="appt-date" className={fieldClass} {...register("date")}>
        ...
        <div>
          ...
          {!date ? (
            <p className="mt-2 text-sm text-muted">Select a date to see open times.</p>
          ) : slotsLoading ? (
            <p className="mt-2 flex items-center gap-2 text-sm text-muted">
              <Clock className="h-4 w-4 animate-pulse" aria-hidden />
              Loading times…
        ...
        <Button
          ...
          disabled={busy || datesLoading || slotsLoading || !selectedTime}
        >
          {busy ? "Confirming appointment…" : "Confirm Appointment"}
        </Button>
      </fieldset>

      <p className="text-center text-xs text-muted">
        Times are shown in your local time zone and checked against our Google Calendar before
        confirmation.
      </p>
    </form>
  );
